...
export default function Login() {
  ...
  // Ref to store the animation instance for cleanup
  ...
  useEffect(() => {
    ...
    return () => {
      mounted = false;
      // Note: This specific library doesn't have a clear dispose method document, 
      // but in React we usually clear the canvas or remove listeners.
      // Since the library attaches to the canvas, we just ensure we don't try to access it after unmount.
    };
  }, []);

  // --- Click to Change Colors Logic ---
  ...
  return (
    <div 
      ...
      onClick={handleBackgroundClick} // Clicking anywhere changes the neon colors
    >
      ...
      <canvas 
        ref={canvasRef} 
        id="canvas" 
        className="fixed top-0 left-0 w-full h-full z-0 pointer-events-none" // pointer-events-none allows clicks to pass through to the div handler
      ...
      <motion.div 
        ...
        onClick={(e) => e.stopPropagation()} // Prevent color change when clicking the form itself
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="space-y-1">
            <div className="flex justify-between items-center ml-1">
              ...
              <a href="#" className="text-xs text-purple-400 hover:text-purple-300 font-bold hover:underline transition-colors">Forgot password?</a>
            </div>
            <div className="relative group">
              <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-slate-500 group-focus-within:text-purple-400 transition-colors">
                <Lock size={20} />
          ...
          <motion.button 
            ...
            type="submit" 
            disabled={isLoading}
            className="w-full py-3.5 px-4 bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-500 hover:to-blue-500 text-white font-bold rounded-xl shadow-[0_0_20px_-5px_rgba(147,51,234,0.5)] flex items-center justify-center gap-2 transition-all disabled:opacity-70"
          >
            {isLoading ? (
        ...
        <div className="mt-8 text-center text-sm text-slate-500 font-medium">
          ...
          <a href="#" className="text-purple-400 font-bold hover:text-purple-300 hover:underline">
            Create an account
          </a>
        </div>
      </motion.div>
